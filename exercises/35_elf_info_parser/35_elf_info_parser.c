...
int main(int argc, char **argv) {
    ...
    if (eh_src.e_ident[EI_DATA] != ELFDATA2LSB && eh_src.e_ident[EI_DATA] != ELFDATA2MSB) {
        fprintf(stderr, "unknown ELF byte order\n");
        fclose(fp);
        return 1;
    }
    ...
    if (eh.e_phnum > 4096 || eh.e_phentsize < sizeof(Elf64_Phdr)) {
    ...
    for (uint16_t i = 0; i < eh.e_phnum; ++i) {
        if (fseek(fp, (long)(eh.e_phoff + (uint64_t)i * eh.e_phentsize), SEEK_SET) != 0) {
            fprintf(stderr, "\nfailed to seek program header\n");
            fclose(fp);
            return 1;
        }
        ...
        if (fread(&ph_src, 1, sizeof(ph_src), fp) != sizeof(ph_src)) {
        ...
        if (ph.p_type == PT_LOAD) {
            ...
            first = 0;
            printf("0x%" PRIx64 "-0x%" PRIx64, ph.p_vaddr, ph.p_vaddr + ph.p_memsz);
        }
    }
